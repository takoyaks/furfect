...
export default function AnnouncementsIndex({ announcements, filters }: Props) {
    ...
    const handleSearch = (e: React.FormEvent) => {
        e.preventDefault();
        router.get(route('admin.cms.announcements.index'), { search }, { preserveState: true });
    };

    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        if (editingItem) {
            post(route('admin.cms.announcements.update', editingItem.id), {
                forceFormData: true,
                onSuccess: () => {
                    setIsOpen(false);
                    reset();
            ...
            post(route('admin.cms.announcements.store'), {
                forceFormData: true,
                onSuccess: () => {
                    setIsOpen(false);
                    reset();
    ...
    const handleDelete = (id: number) => {
        if (confirm('Are you sure you want to delete this announcement?')) {
            router.delete(route('admin.cms.announcements.destroy', id));
        }
    };

    return (
        <AppLayout breadcrumbs={[{ title: 'Content & Pages', href: '#' }, { title: 'Announcements', href: route('admin.cms.announcements.index') }]}>
